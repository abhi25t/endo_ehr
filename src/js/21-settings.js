...
(async function initSettings() {
  ...
  // ── Fetch config defaults from server (HTTP mode only) ──
  ...
  function _restoreProcedure(val) {
    ...
    selectedMainLoc = getLocationsForProcedure()[0];
    _updateLandscapeAvailability();
    if (val === 'colonoscopy') applyDisplayMode('portrait');
  }

  // ── Apply config defaults (before localStorage, so localStorage wins) ──
  if (configDefaults) {
    log('Config defaults from server:', configDefaults);
    if (configDefaults.darkMode === true) toggleDarkMode();
    if (configDefaults.studyType && configDefaults.studyType !== 'retrospective') {
      applyStudyType(configDefaults.studyType);
    }
    if (configDefaults.procedureType && configDefaults.procedureType !== 'endoscopy') {
      _restoreProcedure(configDefaults.procedureType);
    }
    if (configDefaults.displayMode && configDefaults.displayMode !== 'landscape'
        && configDefaults.displayMode !== displayMode) {
      applyDisplayMode(configDefaults.displayMode);
    }
  }

  // ── Restore from localStorage (overrides config defaults) ──
  const savedDark = localStorage.getItem('ehr_darkMode');
  if (savedDark === '1' && !darkMode) toggleDarkMode();
  else if (savedDark === '0' && darkMode) toggleDarkMode();

  const savedStudy = localStorage.getItem('ehr_studyType');
  if (savedStudy && savedStudy !== studyType) {
    applyStudyType(savedStudy);
  }

  const savedProcedure = localStorage.getItem('ehr_procedureType');
  if (savedProcedure && savedProcedure !== procedureType) {
    _restoreProcedure(savedProcedure);
  }
